...
struct Args {
    ...
    /// relative marginal-gain-per-token threshold for adaptive stopping in
    /// pack_regions' pass 2 (E2/issue #4): halt selecting once the best
    /// remaining candidate's marginal gain/token falls below this fraction
    /// of the first-selected candidate's gain/token, even if budget
    /// remains. 0.0 (default) = OFF, current always-fill-the-budget
    /// behavior, byte-identical output to pre-E2.
...
fn main() {
    ...
    if args.adaptive_stop < 0.0 {
        eprintln!("roust: error: --adaptive-stop must be >= 0.0");
        std::process::exit(2);
    }

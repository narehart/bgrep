...
def run_roust(query: str, repo_path: Path, timeout: float, salience_weight: float = 0.0) -> tuple[dict | None, str | None]:
    ...
    if salience_weight != 0.0:
        ...
        # flag) stays byte-identical to pre-E3 argv, see roust-rs/src/
        # main.rs's own `0.0` default for `--salience-weight`.
        argv += ["--salience-weight", str(salience_weight)]
    try:
...
def eval_lite_instance(row: dict, timeout: float, salience_weight: float = 0.0) -> dict:
    instance_id = row["instance_id"]
    ...
    obj, err = run_roust(row["problem_statement"], repo_path, timeout, salience_weight)
    if err:
        rec["error"] = err
        return rec
...
def main() -> None:
    ...
    ap.add_argument("--salience-weight", type=float, default=0.0,
                     help="passthrough to roust's --salience-weight (E3/issue #4); 0.0 (default) matches roust's own default (off)")
    args = ap.parse_args()
    ...
    with args.report.open("w") as fh:
        for i, row in enumerate(rows, 1):
            rec = eval_lite_instance(row, args.timeout, args.salience_weight)

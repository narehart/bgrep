...
def run_roust(query: str, repo_path: Path, timeout: float, chunk_rank: float = 0.0) -> tuple[dict | None, str | None]:
    argv = [str(ROUST_BIN), "--json", "--budget", str(BUDGET), query, str(repo_path)]
    if chunk_rank != 0.0:
        # only appended for non-default values -- default invocation (no
        # flag) stays byte-identical to pre-E5 argv, see roust-rs/src/
        # main.rs's own `0.0` default for `--chunk-rank`.
        argv += ["--chunk-rank", str(chunk_rank)]
    try:
...
def eval_lite_instance(row: dict, timeout: float, chunk_rank: float = 0.0) -> dict:
    instance_id = row["instance_id"]
    ...
    obj, err = run_roust(row["problem_statement"], repo_path, timeout, chunk_rank)
    if err:
        rec["error"] = err
        return rec
...
def main() -> None:
    ...
    ap.add_argument("--chunk-rank", type=float, default=0.0,
                     help="passthrough to roust's --chunk-rank (E5/issue #X); 0.0 (default) matches roust's own default (off)")
    args = ap.parse_args()
    ...
    with args.report.open("w") as fh:
        for i, row in enumerate(rows, 1):
            rec = eval_lite_instance(row, args.timeout, args.chunk_rank)

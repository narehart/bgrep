...
class Corpus:
    ...
    def bm25(
        ...
        multiplied by the implementation-file document prior."""
...
def select_files(
    ...
    owner: dict[str, str] = {}
    import_nbrs: dict[str, list[str]] = {}  # source -> its import-edge candidates
    for s in sources:
        w = bm_n.get(s, 0.0)
        imp: list[str] = []
        for c in list(edges.get(s, ())) + same_dir.get(str(Path(s).parent), []):
...
def pack_regions(
    ...
    for rel in files:
        ...
        if best["tok"] > per_file_cap:
            ...
            seg = "\n".join(seg_lines[:keep])
            best = {**best, "span": (a, a + keep - 1), "text": seg, "tok": count_tokens(seg)}

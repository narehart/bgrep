...
Resume-safe: (instance_id, arm) pairs already present in --out are skipped
on rerun.

Cost safety valve: tracks running $ spend (Anthropic-reported usage tokens
x current Sonnet pricing) and stops launching new (instance, arm) runs once
--budget-cap-usd is reached, so a mis-estimated run can't blow past the
approved budget unattended.
"""
...
from agent import ARMS, MAX_TURNS, MODEL, ROUST_BUDGET, run_agent  # noqa: E402
from common import checkout, load_instances, repo_clone  # noqa: E402
...
def main() -> None:
    ...
    todo = [(i, a) for i in instances for a in arms if (i["instance_id"], a) not in done]
    print(f"{len(instances)} instances x {len(arms)} arms = {total_pairs} pairs, "
    ...
    with out_path.open("a") as out_fh:
        for k, (inst, arm) in enumerate(todo, 1):
            ...
            try:
                ...
                log_path = transcripts_dir / f"{inst['instance_id']}__{arm}.jsonl"
            ...
            print(f"[{k}/{len(todo)}] {inst['instance_id']:44} {arm:11} {status:4} "

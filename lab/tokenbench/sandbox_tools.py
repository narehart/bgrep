"""run_command and read_file -- the shell/file-read tools shared by every
arm that includes them (v2: not every arm gets run_command anymore; see
agent.py's per-arm TOOLS composition).
...
Unchanged from the v1 harness (lab/tokenbench/v1/sandbox_tools.py) -- the
v1->v2 rebuild changed how retrieval is exposed (tool call, not injection)
and the turn cap/prompt, not this baseline toolset.
"""
...
READ_FILE_TOOL = {
    ...
    "description": (
        ...
        f"window around a hit instead. Capped at {MAX_READ_LINES} lines per call; "
...
# Kept for anything that wants "the baseline pair" -- v2's agent.py composes
# its own per-arm tool lists rather than using this constant directly.
